    ...
    while(true) {
        ...
        for (int i = 0; i < jc->MULTI_THREAD_NUM; i++) {
            ...
            while (!(*jc->threadContextsVec[i].mapOutputVector->back().first < *max_pair.first) &&
                ...
                if (jc->threadContextsVec[i].mapOutputVector->empty()) {break;}
            }
        }
        if (!temp_vec->empty()) {
        ...
        if (jc->numIntermediaryElements == 0) {break;}
...
void reducePhase(ThreadContext *context) {
    ...
    jc->state.percentage = 0;
    mutexUnlock(&jc->stateMutex);
    for (size_t ind = jc->helperCounter++; ind < jc->shuffleVec.size(); ind = jc->helperCounter++) {
